...
public class FireProjectile : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (dmg != null)
        {
            dmg.takeDamage(impactDamage); // Apply impact damage
        }

        StatusEffectManager statusEffectManager = other.GetComponent<StatusEffectManager>();
        if (statusEffectManager != null)
        {
            statusEffectManager.ApplyBurn(burnDuration, burnDamage, other);
        }
        Destroy(gameObject);
    }
}